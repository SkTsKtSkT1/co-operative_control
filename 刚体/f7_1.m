function dhdt=f7_1(t,h,n,J1,J2,J3,J4,J5,J6,kG,DG1,DG2,DG3,DG4,DG5,DG6,s_delta,x_delta,y_delta,z_delta,qr,A,LB)
dhdt=zeros(7*n,1);
...
q_deltaall=quaternion(s_delta,x_delta,y_delta,z_delta);
...
%----------------------------------------------------------------------------------------------------------------------------------------
qs=conj(qr).*conj(q_deltaall).*q;               % qs(n*1维的四元数矢量)为qrconj(单个四元数)与q(n*1维的四元数矢量)的姿态误差列向量
...
for j=1:n
    for i=1:n
        qv(j,i)=conj(q_deltaall(j))*conj(q(j))*q(i)*(q_deltaall(i));%conj(q(j)).*(q_deltaall(j)).*conj(q_deltaall(i)).*q(i);        % qv数组的每个元素是qjconj与qi的姿态误差，之所以写成qv(j,i)是为了与文件中的定义相一致
